//
//  BusinessAddressEditPage.swift
//  locarie
//
//  Created by qiuty on 16/11/2024.
//
...
struct BusinessAddressEditPage: View {
  ...
  var body: some View {
    ...
    .task(id: profileUpdateVM.dto) {
      checkDtoInfo()
      if let location = profileUpdateVM.dto.location {
        viewport = .camera(
          center: .init(
...
private extension BusinessAddressEditPage {
  var mapView: some View {
    MapReader { _ in
      Map(viewport: $viewport) {
        if setupDone, let location = profileUpdateVM.dto.location {
  ...
  func onCameraChanged(_ state: CameraChanged) {
    ...
    if !setupDone {
      if profileUpdateVM.dto.location != nil {
        profileUpdateVM.dto.location = .init(latitude: center.latitude, longitude: center.longitude)
      }
      if needUpdating {
      ...
      case .initial, .editing:
        VStack(alignment: .leading, spacing: 10) {
          PlaceSearcher(vm: suggestVM, placeholder: "Type in your address", hint: "Postcode will not work.")
            ...
            .onChange(of: profileUpdateVM.dto.address) { address in
      ...
      case .done:
        HStack(spacing: 10) {
          Image("Map")
            ...
            .scaledToFit()
            .frame(size: 16)
          Text("\(profileUpdateVM.dto.address), \(profileUpdateVM.dto.neighborhood)").lineLimit(1)
          Spacer()
        }
        .frame(height: 48)
      }
...
private extension BusinessAddressEditPage {
  ...
  func setAddress(_ result: PlaceRetrieveDto) {
    profileUpdateVM.dto.address = result.properties.name
  }

  func setLocation(_ result: PlaceRetrieveDto) {
    profileUpdateVM.dto.location = BusinessLocation(
  ...
  func setNeighborhood(_ result: PlaceRetrieveDto) {
    ...
    profileUpdateVM.dto.neighborhood = context.neighborhood?.name
...
extension BusinessAddressEditPage {
