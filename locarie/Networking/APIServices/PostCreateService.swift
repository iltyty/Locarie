...
extension APIServices {
    static func createPost(dto: PostCreateRequestDto) async throws -> ResponseDto<PostCreateResponseDto> {
        do {
            let data = try prepareMultipartFormData(dto, withName: "post", mimeType: "application/json")
    ...
    private static func sendPostCreationRequest(multipartFormData data: MultipartFormData) async throws -> ResponseDto<PostCreateResponseDto> {
        try await AF
