...
@MainActor final class PhotoViewModel: ObservableObject {
    ...
    @MainActor final class ImageAttachment: ObservableObject, Identifiable {
        ...
        enum Status {
            ...
            case finished(Image)  // A status indicating that the app has loaded a photo
            case failed(Error)    // A status indicating that the app failed to load a photo
            var isFailed: Bool {
                return switch self {
                case .failed: true
                default: false
    ...
    // A dictionary that stores previously loaded image attachments for performance
    private var attachmentByIdentifier = [String: ImageAttachment]()
    
    // An array of items for the picker's selected photos
    // On set, this method updates the image attachements for the current selection
    @Published var selection = [PhotosPickerItem]() {
        didSet {
            // Update the attachments according to the current picker selection
            let newAttachments = selection.map { item in
                // Access an existing attachement, if it exists; otherwise, create a new attachment
                attachmentByIdentifier[item.identifier] ?? ImageAttachment(item)
            }
            // Update the saved attachments array for any new attachment loaded in scope
