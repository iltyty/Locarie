...
final class User: Hashable, Equatable, Identifiable, Codable, ObservableObject {
  ...
  init(from decoder: Decoder) throws {
    ...
    type = try UserType(rawValue: container.decode(
      String.self,
      forKey: .type
    )) ?? .plain
    ...
    coverUrl = try container.decodeIfPresent(
      String.self,
      forKey: .coverUrl
    ) ?? ""
    ...
    let openTimeHour = try container.decodeIfPresent(
      Int.self,
      forKey: .openTimeHour
    ) ?? 0
    let openTimeMinute = try container.decodeIfPresent(
      Int.self,
      forKey: .openTimeMinute
    ) ?? 0
    let closeTimeHour = try container.decodeIfPresent(
      Int.self,
      forKey: .closeTimeHour
    ) ?? 0
    let closeTimeMinute = try container.decodeIfPresent(
      Int.self,
      forKey: .closeTimeMinute
    ) ?? 0
    ...
    let latitude = try locationContainer.decodeIfPresent(
      Double.self,
      forKey: .latitude
    ) ?? 0
    let longitude = try locationContainer.decodeIfPresent(
      Double.self,
      forKey: .longitude
    ) ?? 0
  ...
  func encode(to encoder: Encoder) throws {
    ...
    var locationContainer = container.nestedContainer(
      keyedBy: LocationKeys.self,
      forKey: .location
    )
    try locationContainer.encode(
  ...
  enum CodingKeys: String, CodingKey {
    case id, type, username, email, password, avatarUrl, coverUrl,
         introduction,
